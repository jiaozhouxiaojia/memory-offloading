...
long memparse(const char *ptr, char **retptr)
{
	char *endptr;	/* local pointer to end of parsed string */

	long ret = strtol(ptr, &endptr, 0);
...
static int read_config(const char *filename, struct config *config)
{
	...
	fp = fopen(filename, "r");
	if (!fp) {
		LOG_ERROR("Unable to open the configuration file %s\n", filename);
		return -1;
	}
	...
	while (fgets(buffer, sizeof(buffer), fp)) {
		...
		if (sscanf(trimmed_line, "%127[^=]=%127s", key, value) == 2) {
			...
			if (strcmp(trimmed_key, "PSI_THRESHOLD") == 0) {
				...
				config->iterate_min_size = memparse(trimmed_value, &trimmed_value);
				...
				config->interval = memparse(trimmed_value, &trimmed_value);
			} else {
